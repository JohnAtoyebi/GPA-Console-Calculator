...
namespace GPA_CALCULATOR
{
    public class PrintResult
    {
        ...
        public PrintResult(string[] columns)
        {
            if (columns == null || columns.Length == 0)
            {
                Console.WriteLine("Parameter cannot be null nor empty", "columns");
            }
        ...
        public void AddRow(object[] values)
        {
            if (values == null)
            {
                Console.WriteLine("Parameter cannot be null", "values");
            }

            if (values.Length != _columns.Count)
            {
                Console.WriteLine("The number of values in row does not match columns count.");
            }
